...
#define OUT_INT "enxc84d4429bb2d"
#define PROCFS_FILTERS "filter_status_ptype" // Name of the procfs entry for filters
#define PROCFS_DIR "filter_ctech"
...
static int packet_receive(struct sk_buff *skb, struct net_device *dev, struct packet_type *pt, struct net_device *orig_dev) {
    ...
    if (strcmp(dev->name, "lo") == 0){
        ...
        return NET_RX_SUCCESS; /* Don't consume, let kernel continue processing */
    }
    ...
    // Log the packet if it comes from the input interface
    if (strcmp(dev->name, IN_INT) == 0) {
        if (can_pass == 0) {
            return NET_RX_DROP;
        }
    }

    // If the packet is from the output interface, just log it
    if (strcmp(dev->name, OUT_INT) == 0) {
        log_packet(skb);
        return NET_RX_SUCCESS; // Allow the packet to be processed normally
    }

    // Forward the packet to the output device if it matches the filter
    if (can_pass) {
        ...
        out_dev = dev_get_by_name(&init_net, OUT_INT);
        if (out_dev) {
            clone->dev = out_dev; // Set the output device
            dev_queue_xmit(clone); // Transmit the packet
            dev_put(out_dev); // Release the reference to the device
        } else {
            pr_err("Output device not found\n");
            return NET_RX_DROP;
        }
    }
...
static struct packet_type my_packet_type = {
    .type = htons(ETH_P_ALL), // set promic mode to 1;
    .func = packet_receive,
    .dev = NULL,
